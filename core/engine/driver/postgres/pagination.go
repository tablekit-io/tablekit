...
// pageQuery applies the page window to a user query in PostgreSQL's dialect. It
...
//
// Note this wraps rather than injecting LIMIT into the user's own SQL: PostgreSQL
// does not materialize the full inner result — the executor pipelines rows and
// stops the scan once the outer LIMIT is satisfied — so the over-fetch reads at
// most limit+1 rows, not the whole table. (An inner ORDER BY without a matching
// index is the exception: the sort must consume all rows before the outer LIMIT
// can take the top ones.)
