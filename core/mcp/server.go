...
// handlers subpackage — and exposes it as a Streamable HTTP handler. The handler
// is unauthenticated here; the OAuth bearer-token guard is applied by the http
// layer that mounts it on /mcp.
package mcp
...
// newServer builds the MCP server and registers its tools and resources.
...
// for /mcp. The caller is responsible for applying auth.
